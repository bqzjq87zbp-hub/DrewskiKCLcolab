...
export function createPhysicalEasel({ THREE, slot = {}, image, woodImage, sourceSize, index = 0 }) {
  ...
  // These opaque parts never move independently after their authored fit.
  // Bake only their local transforms and concatenate exact geometry/UV data.
  // Keep the leg-bearing timber separate: waterCopy selects it by name.
  const batchMaterials=new Set([timberFront,hardware,brass,dark]);
  const batches=new Map();
  for(const mesh of [...group.children]){
    if(!mesh.isMesh||!batchMaterials.has(mesh.material)||/solid leg|kickstand|front.*leg|rear.*leg/i.test(mesh.name))continue;
  ...
  for(const [material,parts] of batches){
    ...
    for(const mesh of parts){
      ...
      for(const value of baked.attributes.position.array)positions.push(value);
      for(const value of baked.attributes.normal.array)normals.push(value);
      for(const value of baked.attributes.uv.array)uvs.push(value);
      if(baked.index){for(const value of baked.index.array)indices.push(value+offset);}
      else{for(let i=0;i<baked.attributes.position.count;i++)indices.push(i+offset);}
      baked.dispose();
    }
    ...
    batch.name='Batched static '+material.name;
    batch.layers.mask=parts[0].layers.mask;
    batch.castShadow=parts.some(mesh=>mesh.castShadow);
    batch.receiveShadow=parts.some(mesh=>mesh.receiveShadow);
    batch.userData={batchedEaselMeshes:parts.map(mesh=>mesh.name)};
    for(const mesh of parts)group.remove(mesh);
    group.add(batch);
  }
