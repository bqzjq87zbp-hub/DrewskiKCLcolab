import {createWrappedCanvas,homography} from '/pier-v2-20260910/canvas-wrap.js';
...
export function initPhotographicAisle({container,slots,onSelect,onCategory}){
  ...
  const records=slots.map((slot,index)=>{
    ...
    const group=document.createElement('div');group.className='aisle-slot';group.dataset.slotId=slot.id;group.dataset.depth=slot.depth;group.style.zIndex=String(60-slot.depth);const wood=document.createElement('img');wood.className='aisle-wood';wood.src='/pier-v1-20260909/media/easel-composition-reference.jpg';wood.alt='';wood.width=W;wood.height=H;wood.style.clipPath=`url(#${mask.id})`;wood.style.mask=`url(#${exposedWoodMask(index).id})`;group.append(wood);
