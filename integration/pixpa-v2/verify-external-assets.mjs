...
assert.equal(map.files.length,24);assert.equal(map.corsTextureUse,false);
assert.equal(new Set(map.files.map(x=>x.url)).size,24);
const hostedItems=hosted.categories.flatMap(c=>c.items),sourceItems=standalone.categories.flatMap(c=>c.items);
assert.equal(hostedItems.length,94);assert.deepEqual(hosted.categories.map(c=>c.items.length),[21,26,34,13]);
...
console.log(JSON.stringify({status:'PASS',externalDerivatives:24,bytes:map.files.reduce((n,x)=>n+x.bytes,0),photoEntries:94,hostedOutputFiles:assets.length,easelBindingsUnchanged:true}));
