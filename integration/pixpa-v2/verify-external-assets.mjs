...
assert.equal(map.files.length,44);assert.equal(map.corsTextureUse,false);
assert.equal(new Set(map.files.map(x=>x.url)).size,44);
const hostedItems=hosted.categories.flatMap(c=>c.items),sourceItems=standalone.categories.flatMap(c=>c.items);
assert.equal(hostedItems.length,104);assert.deepEqual(hosted.categories.map(c=>c.items.length),[25,27,36,16]);
...
console.log(JSON.stringify({status:'PASS',externalDerivatives:44,bytes:map.files.reduce((n,x)=>n+x.bytes,0),photoEntries:104,hostedOutputFiles:assets.length,easelBindingsUnchanged:true}));
