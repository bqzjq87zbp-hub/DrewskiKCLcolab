...
export function createPhysicalDisplay({viewport,slots,onReady}) {
  ...
  Promise.all([image(sceneImage('/media/underpier-photograph.jpg')),...slots.map(s=>image(sceneImage(s.src)))]).then(([source,...images])=>{
    if(disposed)return;
    environment=createPhotographicEnvironment({THREE,scene,sourceImage:source,sourceDimensions:sceneSourceSize('/media/underpier-photograph.jpg'),calibration:{pierScale:PIER_SCENE_SCALE,maximumTravel:TRAVEL}});
