...
**Living water.** On browsers with WebGPU, a shader overlay (`public/aisle/water-layer.js`) animates refraction and sparkle below the waterline. It is a visual effect over the video, not simulated surf geometry or verified physical easel contact. GPU/image sampling can alter pixels above the waterline; the output is not byte-identical to the base image. The overlay stays hidden while a new bitmap is being prepared, preventing a stale background from covering the current image/easels, including on rapid reversal. Without WebGPU the bundle is not fetched; `?water=off` disables the effect for QA. Still view and operating-system reduced motion keep water animation off, although capable browsers may fetch the bundle and Still can initialize a hidden canvas. The bundle is committed build output; sources live in `tools/water/` (rebuild: `cd tools/water && npm ci --ignore-scripts --no-audit --no-fund && npm run build`, which validates the WGSL and refuses any output that would trip the repository checks).
