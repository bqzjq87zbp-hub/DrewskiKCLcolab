...
**Living water.** On browsers with WebGPU, a shader overlay (`public/aisle/water-layer.js`) keeps the sea below the waterline moving even while scroll is idle, and streams the ripple with the walk as you scroll: subtle refraction plus sparkle that rides the photograph's own wave crests. Pixels above the waterline pass through byte-identical. Without WebGPU the bundle is never fetched and the site behaves exactly as before; `?water=off` disables it for QA; Still view and reduced motion never mount it. The bundle is committed build output; sources live in `tools/water/` (rebuild: `cd tools/water && npm install && npm run build`, which validates the WGSL and refuses any output that would trip the repository checks).
