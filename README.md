...
A self-contained Newport Pier photography portfolio, exported for collaboration. The current gallery contains **111 entries: 25 Branding, 28 Family, 36 Headshot and 22 Coastal**, including the original ten NWPRT photographs within Branding. The accepted seven-photo Google Photos batch and all preceding entries are preserved. The current movement repair separates straight forward/reverse scrolling from explicit looking; the ten easel slots and photographs are unchanged.

## Start here: reproduce the published Pixpa integration

The published source is `eabd8589db7be166352f84b4899090fe913b8dc3`, **Sites version 18**, deployed at 16:39:50 UTC on September 11, 2026. It preserves all 111 photographs, directional glass sheen and full-viewer behavior while correcting the real-phone report of scrolling turning instead of walking. [Actual-public movement QA](integration/pixpa-v2/evidence/straight-walk/public-checks.json) passed 682 checks across nine profiles, including Chrome compositor touch, native desktop wheel and Mac WebKit keyboard input. These are browser profiles, not physical-iPhone certification. [Open the Pixpa gallery](https://kiyonocreativelab.com/pier-gallery?repair=straight-walk-live-20260911) or use these commands with Node.js 20 or newer:
...
Open `http://127.0.0.1:4330/pier-v2-20260910/`. Replace the example output path with a new absolute directory and choose an unused port. The builder verifies **295 gallery files** from this repository. The 58 externally mapped derivatives use native Pixpa Files URLs in the hosted adapter; their exact local copies are included for the standalone build. [Integration README](integration/pixpa-v2/README.md) explains the delivery split, public-verification state, preserved V1 photo URLs, unchanged Pixpa wrapper, rollback and capacity limit.

The plain `npm start` option below serves all 111 entries with local root-relative images. The integration build reproduces the hosted V1/V2 prefixes, ordinary external-image bindings and native Pixpa navigation adapter. Neither command saves or publishes a site, and no private hosting-repository access is required.

[Straight-walk release](integration/pixpa-v2/STRAIGHT-WALK-RELEASE.md) records the current movement repair. [Google seven-photo release](integration/pixpa-v2/GOOGLE7-RELEASE.md) and its [public photo acceptance](integration/pixpa-v2/evidence/google7/public/REPORT.md) record the preserved 111-photo slate. [One-photo release](integration/pixpa-v2/ONE-RELEASE.md), [earlier seven-photo release](integration/pixpa-v2/SEVEN-RELEASE.md), [earlier public acceptance](integration/pixpa-v2/PUBLIC-ACCEPTANCE.md) and [twelve-photo release](integration/pixpa-v2/NEXT12-RELEASE.md) remain historical evidence. Source checks, live browser evidence and physical-device limits are kept distinct. Sheen is retained from version 10; the unrelated external Inquiry legal-link issue is outside this release.
...
The entrance uses Blender-authored dimensional wooden easels and full-source-ratio stretched canvases in a shared perspective scene. The original pier photograph stays on one continuous backdrop at an authored 80m depth, preserving its timber, barnacle and surf contours. Five pairs of artworks share a 7m guided forward/reverse path mapped linearly to scroll position. The backdrop provides shallow whole-image parallax; it is not reconstructed pier geometry. Source photograph files remain unchanged.

Scroll vertically with a mouse/trackpad, or swipe vertically on a phone, to walk immediately. The default view stays ahead; scrolling does not turn it sideways. On supported portrait phones, visible **Look left / Look ahead / Look right** controls hold a chosen heading independently of travel. System reduced motion stops ambient water animation and keeps direct scrolling available. **Still** remains an explicit Menu choice. The menu also contains optional previous/next pair controls. Clear glass is the default; Solid and System appearance remain available. The [straight-walk contract and helper](docs/straight-walk/README.md) document the current behavior; older auto-yaw test records are historical.
...
Three purchased room backgrounds and their six artwork overlays were deliberately excluded because template redistribution rights were not verified. Families now uses its 28-image gallery. See [optional licensed rooms](docs/OPTIONAL-LICENSED-ROOMS.md).
