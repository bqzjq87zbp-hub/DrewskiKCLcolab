> Historical review: these screenshots, shader stages, proposed next steps and test counts predate the current photograph-guided candidate. See [current review](REVIEW.md). The 7.5m/five-plane method described below is no longer the current implementation.
